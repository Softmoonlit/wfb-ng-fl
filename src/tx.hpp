...
private:
    ...
    uint64_t block_idx; // source_local_block_idx
...
struct ReadyDeclarationState {
    enum Phase {
        UNDECLARED,
        DECLARED_WAITING_FOR_TOKEN,
        TOKEN_SEEN_WAITING_FOR_NEXT
    };

    uint8_t node_id;
    bool declaration_in_flight;
    uint64_t last_declare_at_ms;
    uint64_t redeclare_timeout_ms;
    Phase phase;
    std::unique_ptr<TokenAuthorizationDatagramSender> sender;
};
...
void append_token_authorization_stats(std::ostream &out,
