...
void drain_authorization_events(TokenAuthorizationDatagramReceiver *authorization_receiver,
                                TokenAuthorizationState *authorization_state)
{
    if (authorization_receiver == nullptr || authorization_state == nullptr)
    {
        return;
    }
    ...
    for(;;)
    {
        ...
        if (!authorization_receiver->recv_event(&event))
        {
            break;
        }
...
const char *ready_phase_name(ReadyDeclarationState::Phase phase)
{
    switch (phase)
    {
    case ReadyDeclarationState::UNDECLARED:
        return "undeclared";
    case ReadyDeclarationState::DECLARED_WAITING_FOR_TOKEN:
        return "declared_waiting_for_token";
    case ReadyDeclarationState::TOKEN_SEEN_WAITING_FOR_NEXT:
        return "token_seen_waiting_for_next";
    }
...
void log_ready_state_transition(uint8_t node_id,
                                ReadyDeclarationState::Phase before,
                                ReadyDeclarationState::Phase after,
...
bool process_data_packet(Transmitter &t,
                         const uint8_t *buf,
                         size_t size,
                         uint64_t now_ms,
                         TokenAuthorizationState *authorization_state,
...
{
    if (authorization_state != nullptr && !authorization_state->is_authorized(now_ms))
    {
        if (ready_state != nullptr && ready_state->sender)
        {
            const bool should_declare = !ready_state->declaration_in_flight ||
                (ready_state->last_declare_at_ms > 0 &&
                 now_ms >= ready_state->last_declare_at_ms + ready_state->redeclare_timeout_ms);
            ...
            if (should_declare)
            {
                ...
                if (ready_state->sender->send_event(event))
                {
                    ...
                    if (ready_state->phase == ReadyDeclarationState::UNDECLARED)
                    {
                        ready_state->phase = ReadyDeclarationState::DECLARED_WAITING_FOR_TOKEN;
                        log_ready_state_transition(ready_state->node_id,
                                                   phase_before,
                                                   ready_state->phase,
                                                   "first_declare",
                                                   now_ms);
    ...
    return run_when_authorized(authorization_state, now_ms, [&]() {
        const bool sent = t.send_packet(buf, size, 0);
        if (sent && ready_state != nullptr)
        {
            const ReadyDeclarationState::Phase phase_before = ready_state->phase;
            ready_state->declaration_in_flight = false;
            ready_state->phase = ReadyDeclarationState::TOKEN_SEEN_WAITING_FOR_NEXT;
            if (phase_before != ready_state->phase)
            {
                log_ready_state_transition(ready_state->node_id,
                                           phase_before,
                                           ready_state->phase,
...
void append_token_authorization_stats(ostream &out,
                                      const TokenAuthorizationState *authorization_state)
{
    ...
    const TokenAuthorizationCounters &counters = authorization_state->counters();
    out << "\tTOKEN_AUTH\t"
        << counters.accepted_events << ':'
        << counters.rejected_events << ':'
        << counters.authorized_sends << ':'
        << counters.denied_sends;
}

void data_source(unique_ptr<Transmitter> &t,
                 vector<int> &rx_fd,
                 int control_fd,
                 int fec_timeout,
                 bool mirror,
                 int log_interval,
                 TokenAuthorizationState *authorization_state = nullptr,
                 TokenAuthorizationDatagramReceiver *authorization_receiver = nullptr,
                 ReadyDeclarationState *ready_state = nullptr)
{
    int nfds = rx_fd.size();
    assert(nfds > 0);

    const int control_idx = nfds;
    const int auth_idx = nfds + 1;
    const int poll_fds_count = authorization_receiver != nullptr ? nfds + 2 : nfds + 1;
    ...
    fds[control_idx].fd = control_fd;
    fds[control_idx].events = POLLIN;

    if (authorization_receiver != nullptr)
    {
        fds[auth_idx].fd = authorization_receiver->fd();
        fds[auth_idx].events = POLLIN;
    }
    ...
    for(;;)
    {
        ...
        int rc = poll(fds, poll_fds_count, poll_timeout);
        ...
        if (cur_ts >= log_send_ts)  // log timeout expired
        {
            ...
            append_token_authorization_stats(std::cout, authorization_state);
            IPC_MSG("\n");
            IPC_MSG_SEND();
        ...
        if (authorization_receiver != nullptr && rc > 0 && fds[auth_idx].revents & (POLLERR | POLLNVAL))
        {
            throw runtime_error(string_format("socket error: %s", strerror(errno)));
        }

        if (authorization_receiver != nullptr && rc > 0 && fds[auth_idx].revents & POLLIN)
        {
            rc -= 1;
            drain_authorization_events(authorization_receiver, authorization_state);
        }

        // Check control socket first
        if (rc > 0 && fds[control_idx].revents & (POLLERR | POLLNVAL))
        {
            throw runtime_error(string_format("socket error: %s", strerror(errno)));
        }

        if (rc > 0 && fds[control_idx].revents & POLLIN)
        {
            rc -= 1;
            int fd = fds[control_idx].fd;
        ...
        for(int i = _tmp; rc > 0; i = (i + 1) % nfds)
        {
            ...
            if (fds[i].revents & POLLIN)
            {
                ...
                for(;;)
                {
                    ...
                    if (!process_data_packet(*t, buf, rsize, cur_ts, authorization_state, ready_state))
                    {
                        continue;
                    }
...
void local_loop_udp(int argc, char* const* argv, int optind, int rcv_buf, int log_interval,
                    ...
                    int snd_buf_size, uint32_t inject_retries, uint32_t inject_retry_delay, bool token_gate_enabled,
                    uint8_t local_node_id)
{
    ...
    TokenAuthorizationState authorization_state;
    TokenAuthorizationState *authorization_state_ptr = token_gate_enabled ? &authorization_state : nullptr;
    unique_ptr<TokenAuthorizationDatagramReceiver> authorization_receiver;
    unique_ptr<ReadyDeclarationState> ready_state;
    if (token_gate_enabled && udp_port != 0) {
        authorization_receiver.reset(new TokenAuthorizationDatagramReceiver(make_token_authorization_socket_name(std::to_string(udp_port)), rcv_buf));
        if (local_node_id != 0)
        {
            ready_state.reset(new ReadyDeclarationState());
            ready_state->node_id = local_node_id;
            ready_state->declaration_in_flight = false;
            ready_state->last_declare_at_ms = 0;
            ready_state->redeclare_timeout_ms = 4000;
            ready_state->phase = ReadyDeclarationState::UNDECLARED;
            ready_state->sender.reset(new TokenAuthorizationDatagramSender(make_token_ready_socket_name(kDefaultTokenReadySocketBase)));
        }
    }

    int control_fd = open_control_fd(control_port);
    data_source(t, rx_fd, control_fd, fec_timeout, mirror, log_interval, authorization_state_ptr, authorization_receiver.get(), ready_state.get());
}
                     ...
                     uint8_t local_node_id)
{
    ...
    if (token_gate_enabled) {
        authorization_receiver.reset(new TokenAuthorizationDatagramReceiver(make_token_authorization_socket_name(unix_socket), rcv_buf));
        if (local_node_id != 0)
        {
            ready_state.reset(new ReadyDeclarationState());
            ready_state->node_id = local_node_id;
...
#ifndef __WFB_TX_SHARED_LIBRARY__
int main(int argc, char * const *argv)
{
    ...
    while ((opt = getopt(argc, argv, "dI:K:k:n:u:U:p:F:l:B:G:S:L:M:N:D:T:i:e:R:s:f:mVQgP:C:J:E:q:")) != -1) {
        switch (opt) {
        case 'I':
            tx_mode = INJECTOR;
        ...
        case 'q':
        {
            char *end = NULL;
            errno = 0;
            unsigned long parsed_node_id = strtoul(optarg, &end, 10);
            if (errno != 0 || end == NULL || *end != '\0' || parsed_node_id > 255)
            {
                WFB_ERR("Invalid local_node_id: %s\n", optarg);
                exit(1);
            }
        ...
        show_usage:
            WFB_INFO("Local TX: %s [-K tx_key] [-k RS_K] [-n RS_N] { [-u udp_port] | [-U unix_socket] } [-R rcv_buf] [-p radio_port]\n"
                     ...
                     "             [-g] [-q local_node_id] [-P fwmark] [-J inject_retries] [-E inject_retry_delay] [-C control_port] interface1 [interface2] ...\n",
                    argv[0]);
            ...
            WFB_INFO("TX injector: %s -I port [-Q] [-R rcv_buf] [-l log_interval] interface1 [interface2] ...\n",
                    argv[0]);
            WFB_INFO("Default: K='%s', k=%d, n=%d, fec_delay=%u [us], udp_port=%d, link_id=0x%06x, radio_port=%u, epoch=%" PRIu64 ", bandwidth=%d guard_interval=%s stbc=%d ldpc=%d mcs_index=%d vht_nss=%d, vht_mode=%d, fec_timeout=%d, log_interval=%d, rcv_buf=system_default, snd_buf=system_default, frame_type=data, mirror=false, use_qdisc=false, token_gate=false, local_node_id=%u, ready_socket=%s.ready, fwmark=%u, control_port=%d, inject_retries=%u, inject_retry_delay=%u\n",
                     keypair.c_str(), k, n, fec_delay, udp_port, link_id, radio_port, epoch, bandwidth, short_gi ? "short" : "long", stbc, ldpc, mcs_index, vht_nss, vht_mode, fec_timeout, log_interval, local_node_id, kDefaultTokenReadySocketBase, fwmark, control_port, inject_retries, inject_retry_delay);
    ...
    try
    {
        if (token_gate_enabled && local_node_id == 0)
        {
            throw runtime_error("Token gate enabled requires -q local_node_id");
        }
        ...
        case LOCAL:
            if (unix_socket != NULL)
            {
                local_loop_unix(argc, argv, optind, rcv_buf, log_interval,
                                ...
                                snd_buf, inject_retries, inject_retry_delay, token_gate_enabled, local_node_id);
            }
            else
            {
