...
void Aggregator::dump_stats(void)
{
    ...
    std::set<uint8_t> nodes_to_report = known_client_node_ids_;
    for(size_t i = 1; i < 256; i++)
    {
        if (source_states_[i] != NULL)
        {
            nodes_to_report.insert(static_cast<uint8_t>(i));
        }
    }
...
void Aggregator::clear_stats(void)
{
    antenna_stat.clear();
    count_p_all = 0;
    count_b_all = 0;
...
bool Aggregator::get_source_stats(uint8_t source_node, RxSourceStats *stats) const
{
    if (source_node == 0 || source_states_[source_node] == NULL)
    {
        return false;
    }
    if (stats != NULL)
...
int main(int argc, char* const *argv)
{
    ...
    while ((opt = getopt(argc, argv, "K:N:m:fa:c:u:U:p:l:i:e:R:s:k:n:")) != -1) {
        ...
        case 'N':
        {
            int parsed_node_id = atoi(optarg);
            if (parsed_node_id < 0 || parsed_node_id > 255)
            {
                WFB_ERR("Invalid local node_id: %s\n", optarg);
                exit(1);
            }
    ...
    try
    {
        ...
        unique_ptr<TokenEventDatagramListener> token_event_listener;
