...
class GenericProcessProtocol(ProcessProtocol):
    def __init__(self, cmd, proc_id):
        self.cmd = cmd
    ...
    def connectionMade(self):
        log.msg('Started %s' % (self.proc_id,))
    ...
    def processEnded(self, status):
        ...
        log.msg('Stopped %s with code %s' % (self.proc_id, rc))
...
class TokenGateDualClientTestCase(unittest.TestCase):
    @defer.inlineCallbacks
    def setUp(self):
        bindir = os.path.join(os.path.dirname(__file__), '../..')
        yield call_and_check_rc(os.path.join(bindir, 'wfb_keygen'))

        self.tx1_in = UDP_TXRX(('127.0.0.1', 11003))
        self.tx2_in = UDP_TXRX(('127.0.0.1', 12003))
        self.tx1_out = UDP_TXRX(('127.0.0.1', 11001))
        self.tx2_out = UDP_TXRX(('127.0.0.1', 12001))

        self.tx1_out_ep = reactor.listenUDP(11004, self.tx1_out)
        self.tx2_out_ep = reactor.listenUDP(12004, self.tx2_out)
        self.tx1_in_ep = reactor.listenUDP(11001, self.tx1_in)
        self.tx2_in_ep = reactor.listenUDP(12001, self.tx2_in)

        link_id = int.from_bytes(os.urandom(3), 'big')
        epoch = int(time.time())

        cmd_tx1 = [os.path.join(bindir, 'wfb_tx'), '-K', 'gs.key', '-u', '11003', '-D', '11004', '-T', '30', '-F', '3000',
                   '-g', '-q', '1', '-i', str(link_id), '-e', str(epoch), '-R', str(512 * 1024), '-s', str(512 * 1024), 'wlan0']
        cmd_tx2 = [os.path.join(bindir, 'wfb_tx'), '-K', 'gs.key', '-u', '12003', '-D', '12004', '-T', '30', '-F', '3000',
                   '-g', '-q', '2', '-i', str(link_id), '-e', str(epoch), '-R', str(512 * 1024), '-s', str(512 * 1024), 'wlan0']
        cmd_scheduler = [os.path.join(bindir, 'wfb_token_scheduler'), '-n', '1,2', '-d', '100', '-g', '10', '-s', '1:11003,2:12003']

        ap = FakeAntennaProtocol()
        self.tx1_pp = TXProtocol(ap, cmd_tx1, 'token tx1')
        self.tx2_pp = TXProtocol(ap, cmd_tx2, 'token tx2')
        self.scheduler_pp = GenericProcessProtocol(cmd_scheduler, 'token scheduler')

        self.tx1_pp.start().addErrback(lambda f: f.trap('twisted.internet.error.ProcessTerminated'))
        self.tx2_pp.start().addErrback(lambda f: f.trap('twisted.internet.error.ProcessTerminated'))
        self.scheduler_pp.start().addErrback(lambda f: f.trap('twisted.internet.error.ProcessTerminated'))

        yield df_sleep(0.2)

    @defer.inlineCallbacks
    def tearDown(self):
        for proc in (self.tx1_pp, self.tx2_pp, self.scheduler_pp):
            if getattr(proc, 'transport', None) is not None:
                proc.transport.signalProcess('KILL')

        self.tx1_out_ep.stopListening()
        self.tx2_out_ep.stopListening()
        self.tx1_in_ep.stopListening()
        self.tx2_in_ep.stopListening()
        yield df_sleep(0.1)

    @defer.inlineCallbacks
    def test_dual_clients_rotate_after_ready_declarations(self):
        self.assertEqual(len(self.tx1_out.rxq), 0)
        self.assertEqual(len(self.tx2_out.rxq), 0)

        for i in range(20):
            self.tx1_in.send_msg(b'client1-%02d' % (i,))
            self.tx2_in.send_msg(b'client2-%02d' % (i,))
            yield df_sleep(0.02)

        yield df_sleep(0.8)

        self.assertGreater(len(self.tx1_out.rxq), 0)
        self.assertGreater(len(self.tx2_out.rxq), 0)

        scheduler_log = bytes(self.scheduler_pp.stdout).decode('utf-8', errors='replace')
        self.assertIn('grant seq=0 node_id=1 duration_ms=100 guard_interval_ms=10', scheduler_log)
        self.assertIn('grant seq=1 node_id=2 duration_ms=100 guard_interval_ms=10', scheduler_log)
