...
只接受整数 `schema_version: 1`。必需字段缺失、重复、类型错误、值域非法或出现未知字段时失败；字段顺序无语义。改变既有字段含义或作不兼容变更必须新增 schema 版本，不能静默重定义。manifest 不签名、不加密；`sha256` 只表示文件完整性。
