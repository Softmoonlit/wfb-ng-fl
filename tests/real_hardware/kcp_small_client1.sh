#!/bin/bash
# 三设备 KCP small 手动验收：client1 端脚本
...
CLIENT_IFACE="${CLIENT_IFACE:-wlxfca386b38672}"
CLIENT_PORT="${CLIENT_PORT:-5602}"
NODE_ID="${NODE_ID:-1}"
...
FEC_K="${FEC_K:-12}"
FEC_N="${FEC_N:-12}"
LOG_DIR="${LOG_DIR:-$ROOT/tests/logs/manual_kcp_client1_$(date +%Y%m%d_%H%M%S)}"
SOURCE_FILE="${SOURCE_FILE:-$LOG_DIR/kcp_small_client1.bin}"
