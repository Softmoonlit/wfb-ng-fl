...
1. **服务端常驻协同守护引擎与本地专用 REST IPC (Server Daemon & Local IPC)**：
   Server 部署开机常驻守护服务 `wfb-fl-server-daemon`，开机后独占纳管无线网卡并持续在后台监听客户端心跳，维护全局客户端实时在线拓扑视界（`NodeHorizonRegistry`）。对外暴露本地回环专用 HTTP REST API（`http://127.0.0.1:9090`），为 CLI 工具与 Web 后端提供无冲突的常态化状态推流、扫频触发与任务下发通道。
...
3. **控制信令面与数据面正交分离的双平面架构 (Dual-Plane Protocol)**：
   - **控制信令面（小 JSON，对称轻量 UDP）**：下行由 Server 向 `10.80.0.255:9000` 广播任务级指令，各 Client 向 `10.80.0.1:9001` 单播发送心跳保活与状态跃迁（自然交错零碰撞），Server 仅做单一单播回执（`{"ack": true}`），空口保持绝对静默，享底层 FEC 8/14 保护；
...
6. **纯底层网卡参数预设分级与扫频探路 (Radio Presets & Spectrum Survey)**：
   - 固化 FEC 为全局基准 `8/14`；提供 `robust`（MCS 2 HT20）、`standard`（MCS 3 HT40+，系统基准）、`performance`（MCS 5 HT40+）三档纯网卡参数预设；
...
12. 作为 Client 守护进程，我希望在网卡未插好时保持内部安全挂起轮询，以便在操作者插入 USB 网卡后能自动无感接管。
...
15. 作为 Server 协同守护引擎，我希望对外暴露统一本地 HTTP REST 接口（`127.0.0.1:9090`），以便 CLI 工具与 Web 后端能无冲突地发起查询与任务控制。
16. 作为 链路控制面，我希望所有心跳、状态汇报与任务信令采用轻量 UDP 报文承载，各客户端自然交错单播上行，以便杜绝空口广播唤醒引发的同频对撞撕裂（Collision）。
...
- **服务端常驻守护引擎与本地专用 REST IPC (Server Daemon & Local IPC)**：
  ...
  - 拉起底层 `wfb_v6_uplink` 时，一次性全量预置平台支持的 1~10 号节点槽位（`--known-clients 1..10` 及其静态 `tun_ip` 映射），确保后续任何新节点接入时均可立即被 C++ Token 调度器授予 Grant 并建立单播下行路由，无需重启底座；
  ...
  - 对外提供统一的本地专用 REST API（`http://127.0.0.1:9090`），支持的 IPC 动作：
    ...
    - `POST /api/v1/jobs/start`：提交包含预设、模式、参与节点与模型路径的仿真作业（触发前置门禁）；
...
- **轻量对称 UDP 控制信令平面 (Symmetric UDP Control Plane)**：
  - **下行方向（Server -> Clients）**：
    - 单播极简回执：收到任一客户端心跳后，即刻单播回复单一布尔确认 `HEARTBEAT_ACK`（仅包含 `{"ack": true}`，空口不足 20 字节）；若检测到该客户端自身预设与集群当前生效预设不一致，自动下发对齐指令；
  ...
  - **上行方向（Client -> Server，单播）**：Client 向 `10.80.0.1:9001` 发送 JSON 数据报：
    - `NODE_HEARTBEAT`：全流程细粒度状态载体（携带 `node_id`、`state`、`elapsed_ms`、`current_channel`、`preset` 与错误码）；
...
- **两阶段防脑裂租约式看门狗自愈协议 (Lease-based Anti-Lockup Watchdog)**：
  ...
  3. **租约刷新与终审定案**：
     ...
     - **局部失败自愈（防脑裂）**：若 Server 在 10 秒内未收齐，Server 立即停止在 149 上发心跳并退回 Channel 157；新信道上的客户端由于 Server 消失，15 秒租约耗尽，看门狗强制将物理网卡切回 Channel 157 并启动三级寻频自愈。三机最终整齐汇合于基准信道 157！
...
- **5GHz 频段物理规划、预设与速率安全区间**：
  - **40 MHz 宽频（HT40+）**：受频段法则制约，全频段物理上仅有两个合法独立频块：**Channel 157**（默认基准，占用 157+161，中心 159）与 **Channel 149**（占用 149+153，中心 151）；
  - **20 MHz 窄频（HT20）**：包含 **149、153、157、165** 四个独立信道；
  - **永久黑名单**：Channel 161 驱动越界崩溃缺陷，永久禁止单独配置或作为主信道；
  - 继承 ADR-0011：固化 FEC 8/14；内置 `robust`（MCS 2 HT20）、`standard`（MCS 3 HT40+，默认基准）、`performance`（MCS 5 HT40+）三档底层网卡预设；
...
- **测试设计原则 (Seams at the highest level)**：
  ...
  - 保证在无物理无线网卡的开发与 CI 环境中，通过虚拟网络接口对与 Mock 套接字能够 100% 完整测试协议、状态机、两军问题防虚假就绪与看门狗逻辑。
...
- **五大核心测试接缝 (Primary Testing Seams)**：
  1. **Server 常驻守护与 Local REST IPC 接缝 (Server Daemon REST IPC Seam)**：
     ...
     - 断言 `GET /api/v1/status` 能正确反映从模拟 UDP 心跳中解析出的客户端细粒度状态与耗时；
     - 断言 `POST /api/v1/jobs/start` 的四项前置门禁（状态未确认为 `IDLE` 拦截、模型不合法拦截、速率超限拦截）。
